...
2. Detect available JDKs: check `JAVA_HOME`, run `java -version`, scan common JDK install paths.
3. Detect build tools: check for Maven/Gradle wrappers in project root, run `mvn --version` or `gradle --version`.
...
3. **Determine migration guides**: For each legacy package, find the appropriate guide from [Migration Guidelines](./INSTRUCTION.md). Consult [Migration Guidelines](./INSTRUCTION.md) for package mappings and migration guides. Record which guide URL applies to each legacy package.
...
Update `pom.xml` or `build.gradle` to replace legacy dependencies with modern equivalents. Follow the pom.xml/build.gradle examples in [Migration Guidelines](./INSTRUCTION.md).

**Important**: `azure-resourcemanager-xx` should have groupId `com.azure.resourcemanager` instead of `com.azure`.

Look up the latest stable version on Maven Central and proceed with migration.

### Step 2: Update Source Code

- Migrate each source file that contains legacy SDK packages.
- Follow the code migration guidelines and samples in [Migration Guidelines](./INSTRUCTION.md).
- Determine legacy SDK artifacts according to the files identified in Phase 2, find suitable migration guides in the [Package-Specific Migration Guides](./INSTRUCTION.md#package-specific-migration-guides) and follow the guides whenever possible.
...
Apply the validation checklist from [Migration Guidelines](./INSTRUCTION.md#validation). Verify all goals are met:
